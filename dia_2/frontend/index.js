...
async function crearProductoRequest(body){
    // stringfy convierte eel objeto de JS a JSON para que lo pueda recibir el backend
    const solicitud = await fetch('http://127.0.0.1:5000/producto', {method:'POST', body:JSON.stringify(body), headers:{
        'Content-Type' : 'application/json',
    }})
    // los headers sirven para enviar un encabezado de la peticion es decir gracias a estos el backend sabra
    //que la informacion recibe, cuestiones de autenticacion, el origen de la peticion entre otros
