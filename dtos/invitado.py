...
class RegistrarInvitadoDTO(Schema):
    # validamos que el dni sea entre 8 y 11 caracteres u sea puro numero
    dni = fields.String(
        required=True, validate=validate.Regexp(regex='^(?:\d{8}|\d{11})$', 
    ...
    telefono = fields.String(
        required=True, validate=validate.Regexp(
            regex='^\d{9}$', 
            error='el telefono tiene que tener 9 cnumeros'),
        error_messages={
